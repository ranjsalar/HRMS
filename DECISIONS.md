...
**Visibility gating decision, made explicit rather than assumed:** `CreateProjectForm` is shown for `company_admin` only — role-based, in `projects/page.tsx` — matching `AddEmployeeForm`'s existing precedent for the *identical* RBAC shape (`projects:create`, like `employees:create`, is admin-by-default/manager-opt-in-only, and this codebase's established answer to "does the UI need to detect an opt-in grant" is no: the button is admin-only, the backend's own 403 is the real boundary for anyone else, same "hiding a control is a UX nicety, not a security boundary" principle `AddEmployeeForm`'s own comment states directly). Edit + member management, by contrast, are shown to `company_admin` **or** `manager` — `projects:edit` at `own_department` is a *default* grant (no opt-in), so unlike create, every manager can genuinely use these controls on any project already visible to them. Archive stays `company_admin`-only, matching `projects:delete` never being a default manager grant either. All three components take `canManage`/`canArchive` as plain booleans, not internal `useAuth()` reads — same prop-over-context preference as `AddEmployeeForm`'s `isAdmin`.

**A real bug this step's own test caught, not a pre-existing one:** the first integration-test draft removed the newly-added project member (deptA) *before* testing manager visibility — but a manager's `own_department` scope for `Project` requires a real member from their managed department to already be present (step 3's own deliberate design). Removing it first made the manager's view 404 for a genuinely correct backend reason, which looked like a bug in the new code until traced back to test ordering. Fixed by reordering: add member → prove manager sees it → prove a plain read-only render shows no controls → remove member → archive. Documented here because it's a second real instance of the same "manager can't act on what has no matching member yet" edge case flagged (and accepted as deferred) back in step 3 — this time it shaped a test's ordering, not a feature gap.

**A second, narrower bug, in the test itself:** the add-member assertion originally chained `await screen.findByText(...)).toBeInTheDocument()` — `findByText` resolves once to a specific node; between that resolution and the very next line, two overlapping re-renders (the picker's own reset, the parent's `onChanged`-triggered reload) detached that specific node, failing `toBeInTheDocument()` even though the member had genuinely been added (proven by every later test in the file, which fetches fresh and finds it fine). Fixed by switching to `waitFor(() => expect(screen.getByText(...)).toBeInTheDocument())`, which re-queries the live DOM on every poll instead of asserting against a single resolved-then-possibly-stale reference — the more robust pattern already used everywhere else in this file, now used consistently.

**RBAC boundary proof (`project-management.integration.spec.tsx`, 7 tests):** admin creates/edits/adds a member/removes a member/archives, all through the real rendered UI, not direct API calls; manager sees and can edit a project once it has a real member from their department, but never sees an archive control; a plain read-only render (no `canManage`/`canArchive`) shows no management controls at all; archive is soft — status becomes "Cancelled," the row stays readable afterward.

## 2026-08-01 — Projects module, step 6.0+6.1: frontend foundation + Project list/detail (view-only), combined into one checkpoint

Proposed a 6.0–6.5 sub-step breakdown for the frontend (mirroring HR self-service's 9.0–9.6), approved by the founder along with a "plain list, not drag-and-drop Kanban" call for the later task-board sub-step (`Projects-Module-Plan.md` §4 left that explicitly as a frontend judgment call). Started building 6.0 (foundation: API clients, locale scaffolding, nav entry, route skeleton) as its own thing, then **merged it into 6.1 once actually building it** — unlike the original step 9.0 (which built test-runner/API-client/auth/i18n infrastructure that did not exist yet in a bare `apps/web` scaffold), every one of those pieces already exists in this app now. A standalone "API client + locale keys, no consuming component" commit would have had nothing genuinely testable or reviewable of its own in this codebase's established style — every existing frontend feature's API client functions are exercised indirectly through the component that calls them (e.g. `team-api.ts`'s `createEmployee` has no test of its own; `add-employee.integration.spec.tsx` exercises it through `AddEmployeeForm`), not tested standalone. Flagging this explicitly rather than silently delivering a different-shaped checkpoint than what was approved.

Delivered together: `projects-api.ts` (fetchProjects/fetchProject), `ProjectList`/`ProjectDetail` components, `nav.projects` + a `projects` locale namespace in all three locale files, `/projects` and `/projects/[id]` routes (this app's first dynamic route segment — `ProjectDetail` takes `projectId` as a prop rather than reading `useParams()` itself, same "prop over internal context/routing read" preference already established for `AddEmployeeForm`'s `isAdmin` prop, keeping the component testable without Next.js routing context).

**Real-backend integration test** (`projects.integration.spec.tsx`) proves the three-tier scope model through the actual UI, not just re-asserting the backend's own boundary — creates two real projects via the real API in `beforeAll` (one with a member from the shared fixture company's managed department, one with a member from its "other" department), then renders `ProjectList` under each of the three real fixture sessions (admin/manager/employee) and asserts on the rendered DOM. No dedicated locale-rendering spec file added for this feature (unlike `LoginPage`/`LeaveSubmitForm`'s `.locales.spec.tsx` files) — matches the lighter precedent `TeamList`/`CompanyOverview` already established: RTL/translation correctness is structural (the same `dir`/`t()` from `useTranslation()` used everywhere), and the existing `locale-parity.spec.ts` already catches key-completeness regressions across all three locale files.

**Environment note, not a code issue:** Docker Desktop stopped again mid-session (a recurring environment quirk already documented earlier in this build) — restarted via `Start-Service com.docker.service` + relaunching Docker Desktop, same recovery as before. Also confirmed (again) that `leave-approvals.integration.spec.tsx`'s shared fixture employee has more accumulated leave-request pollution on this long-lived local dev DB (this time a 409 "overlaps an existing" on submit, rather than the previously-documented balance-exhaustion symptom) — same root cause class as already flagged multiple times, still confirmed CI-irrelevant (fresh DB every run), not fixed, not this module's concern.
...
**Logging is always self-authored, unconditionally, regardless of role** — like `AttendanceRecord` clock-in, there is no "log time on behalf of someone else." `TaskTimeEntriesService.log()` always resolves the caller's own `Employee` row and requires them to be the task's assignee (the same "on tasks assigned to them" rule as `TasksService.updateStatus`'s `self`-scope path) — this restriction is NOT scope-tiered the way visibility is; an admin logging their own time follows the identical assignee check an employee does. Proved: a project member who is a genuine member but not the assignee gets a 404 attempting to log, same as attempting a status update.

**`own_department` for TaskTimeEntry scopes by the *logging employee's* department — deliberately different from Task's own project-membership-based `own_department` rule, even though both live under the same `"projects"` RBAC module.** Mirrors `AttendanceService.teamTimesheet`'s exact pattern (scope by `employee.departmentId`, not by task/project relationships) rather than reusing `ProjectsService`/`TasksService`'s membership-based `scopeWhere`. This produces a real, intentional gap worth stating plainly: a manager can *see* a cross-departmental task (because its project has a member from their department) while seeing *zero* time entries logged on it, if the actual assignee who logged that time belongs to a different department. Proved directly in `task-time-entries.e2e-spec.ts` with a task assigned to a deptB employee inside an otherwise deptA-visible project — the manager can `GET` the task (200) but lists zero entries on it.

**Decimal serialization**: `TaskTimeEntry.hours` returns as `Prisma.Decimal`'s own `.toJSON()` string form (e.g., `"3.5"`, not `"3.50"` — no manual padding), same pattern already established for `PayrollRegionRule.overtimeMultiplier`; the DTO validates with `@IsNumber()`/`@Type(() => Number)` and Prisma accepts the plain number on write.

**RBAC boundary proof (`task-time-entries.e2e-spec.ts`, 12 tests):** the assignee logs successfully with a real audit row; a non-assignee project member is rejected (404); logging on a fully out-of-scope task is rejected (404) for anyone; an over-ceiling `hours` value (>24, a sanity bound, not a real workday-length rule — this module has no such policy to enforce) is rejected client-side (400); admin sees every entry; manager sees entries from their own department's employees but not another department's, even on a task they can otherwise view; an employee sees only their own entries, even on a task they're a member of but didn't log time on.

## 2026-08-01 — Projects module, step 4: Task CRUD, with the employee status-only allow-list

Fourth of the module's seven confirmed build steps. `TasksController`/`TasksService` added to the existing `ProjectsModule` (one RBAC module, `"projects"`, covers both entities per `Projects-Module-Plan.md` §3 — no separate `TasksModule`; `TasksService` depends directly on `ProjectsService`). Routes: `GET /tasks`, `GET /tasks/:id`, `POST /tasks`, `PATCH /tasks/:id` (general — title/description/status/assigneeId/dueDate), `PATCH /tasks/:id/status` (the employee allow-list route), `DELETE /tasks/:id` (real hard delete — the plan's own wording is "delete" for Task vs "archive" for Project, a deliberate difference honored here rather than flattened to match Project's soft-archive convention; `TaskStatus` has no cancelled/archived value to soft-delete into anyway).

**Task's `self` scope is asymmetric — VIEW is broader than WRITE, a genuine first in this codebase.** Every prior module's scope has been symmetric (the same `where` clause gates both read and write at a given scope). Task's view scope is "a project I'm a member of, OR a task assigned to me" (`Projects-Module-Plan.md` §3's own wording, implemented as a Prisma `OR`), but its write scope is narrower: only a task's *assignee* may touch it, and only its `status` field, via the dedicated `PATCH /tasks/:id/status` route and `UpdateTaskStatusDto` (structurally cannot carry any other field — same belt-and-suspenders shape as `UpdateOwnEmployeeDto`). Proved directly: a project member who is NOT the assignee can see the task in `GET /tasks` but gets a 404 attempting to update its status.

**Retroactively hardened `ProjectsService.create()` with the same `self`-scope guard `update()`/`archive()` already had** (step 3 left this one path unguarded, relying only on the default matrix never granting `projects:create` to an employee) — applied consistently now because `TasksService.create()` needed the identical guard from the start, and leaving Project's asymmetric would have been inconsistent for no reason. Flagged explicitly rather than left silent, since step 3 was already reported closed.

**RBAC boundary proof (`tasks.e2e-spec.ts`, 14 tests):** admin sees/edits everything; manager (`own_department`) sees and edits only tasks in a project with a member from their managed department, 404s on the other; employee sees a task via either membership OR assignment but cannot create, delete, or edit task details via the general route regardless of assignment (403, service-level, matching Project's precedent); the actual assignee CAN update status via the dedicated route, a non-assignee project member cannot (404); a smuggled extra field on the status route is rejected 400 by the DTO itself, not silently dropped; a real hard delete leaves no row behind, confirmed by direct DB read.

## 2026-08-01 — Projects module, step 3: Project + ProjectMember CRUD, with the three-tier scope model e2e-proven for real

Third of the module's seven confirmed build steps (`Projects-Module-Plan.md` §6). New `ProjectsModule` (`src/modules/projects/`): `ProjectsController`/`ProjectsService`/DTOs, wired into `app.module.ts`. Routes: `GET /projects`, `GET /projects/:id`, `POST /projects`, `PATCH /projects/:id`, `DELETE /projects/:id` (soft archive — status → `cancelled`, never a hard delete, same convention as `Employee`), `POST /projects/:id/members`, `DELETE /projects/:id/members/:employeeId`. Gated by the existing `@RequirePermission`/`RbacGuard`/`CurrentPermissionScope` machinery — no new permission mechanism, exactly as planned.

**`own_department` scope resolution is genuinely different from every prior module's:** `Project` has no home-department FK of its own (deliberate — projects are commonly cross-departmental, per the plan's §1). `ProjectsService.scopeWhere()` resolves it as `{ members: { some: { employee: { departmentId: managedDepartmentId } } } }` — a project is in a manager's scope if ANY member belongs to a department they manage, not if the project "belongs" to that department. `self` scope is the same shape, filtered to the caller's own `employeeId`. Both reuse `EmployeesService.managedDepartmentId`/`findOwn` (already exported for exactly this kind of cross-module scope resolution — same pattern `AttendanceService`/`DocumentsService` follow), rather than duplicating that resolution logic.

**`self` scope is rejected outright, in the service layer, for every write** (update/archive/add-member/remove-member) — `Projects-Module-Plan.md` §3 is explicit that employees never create or manage projects, only their own tasks (a later step). This matters concretely because the *default* employee grant seeded in step 2 was `projects:edit` at `self` scope — seeded in anticipation of task-status/time-entry edits — which means `RbacGuard` alone does NOT block an employee from reaching `ProjectsService.update()`. The service-level `if (scope === "self") throw ForbiddenException(...)` is what actually enforces the boundary. Proved directly in `projects.e2e-spec.ts`: an employee who genuinely holds `projects:edit` at `self` scope still gets a 403 attempting to update a project they're a real member of — defense-in-depth verified, not assumed.

**Known, deliberate edge case, not fixed (out of scope for what the plan asked):** a manager who creates a project (once granted the `projects:create` opt-in) cannot immediately see or manage it under their own `own_department` scope, because a freshly created project has zero members yet, and `own_department` visibility requires at least one member from a managed department. There is no per-project "home department" to auto-satisfy the way `EmployeesService.create()` forces `departmentId = managedDepartmentId` for a manager creating an employee — `Project` has no equivalent single field to force. Resolving this (e.g., auto-adding the creator as a member) would be inventing behavior the plan never specified; flagging it here rather than silently building around it, matching this project's standing "flag, don't invent" discipline. In practice this only matters once manager-side project creation ships in the frontend (step 6) — until then, `company_admin` is the only role that creates projects by default, and an admin's `all` scope has no such gap.

**RBAC boundary proof (`projects.e2e-spec.ts`, 19 tests):** company_admin sees/edits every project; manager (default `own_department` grant) sees and edits only the project with a member from their managed department, 404s on the other; manager cannot create without an explicit opt-in grant (403), then can once granted; employee (`self`) sees only the project they're a member of, 404s on the other, cannot create (no default grant) or archive (no default `projects:delete` grant), and — per the defense-in-depth point above — cannot update or manage membership despite holding `projects:edit` at `self` scope. Also covers: duplicate membership rejected with 409 (not a silent no-op), a real `AuditLog` row for project creation, and archive leaving the row readable afterward (soft, not hard).
